...
# # ================== FILE SAVING ==================
# def extract_filename_from_query(user_query: str) -> str | None:
#     """Extract user-specified OUTPUT filename from query e.g. 'save in health.md'
#     Ignores CSV input files like customers.csv since those are inputs not outputs.
#     """
#     # only match md, txt, py, json — NOT csv (csv is always input, not output)
...
#     if blocks:
#         return "\n\n".join(blocks).strip()
#     # if no code blocks found, return as is if it looks like code
...
#     """
#     Python backend handles all file saving.
#     Agents only generate text — this function saves everything.
#     """
#     os.makedirs(OUTPUT_DIR, exist_ok=True)

#     user_filename = extract_filename_from_query(user_query)
#     is_coding_task = "Coder" in global_context and global_context["Coder"] != "Skipped coder"

#     # ── 1. Save Reporter output ──────────────────────────────────────────────
...
#     # ── 2. Save Coder output as .py ──────────────────────────────────────────
#     if is_coding_task:
#         coder_output = str(global_context["Coder"])
#         code = extract_code_blocks(coder_output)

#         if code:
#             # Try to get meaningful filename from query
#             code_name = "output.py"
#             words = user_query.lower().split()
#             for i, word in enumerate(words):
#                 if word in ("factorial", "fibonacci", "sort", "search", "calculator"):
#                     code_name = f"{word}.py"
#                     break

#             code_path = os.path.join(OUTPUT_DIR, code_name)
#             with open(code_path, "w", encoding="utf-8") as f:
#                 f.write(f"# Generated by NEXUS AI\n# Query: {user_query}\n\n")
#                 f.write(code)
#             print(f" Saved code → {code_path}")

#     # ── 3. Save Analyst output if no Reporter ────────────────────────────────
#     if "Analyst" in global_context and "Reporter" not in global_context:
#         analyst_output = str(global_context["Analyst"])
...
#     # Skip coder for planning tasks
#     if agent_name == "Coder" and any(word in user_query.lower() for word in ["plan", "strategy", "idea"]):
...
#         # Strong validation for coder
#         if agent_name == "Coder" and len(output.strip()) < 5:
